#!/usr/bin/env python3
"""Count component composition in spike-centered contact-shell PDB files."""
...
from rav_components import COUNT_COLUMNS, classify_segment, load_mucin_pairs, mucin_subtype, spike_group

RESIDUE_COMPONENTS = {"DPPG", "DPPC", "CHL", "CAL", "CLA", "SOD", "MG", "POT"}

def process_pdb(pdb_file: Path, selected_spike_group: str, mucin_pairs: dict[str, str]):
    ...
    with pdb_file.open() as fh:
        for line in fh:
            ...
            # Exclude the reference spike itself. Count only other spikes if present.
            if comp == "Spike":
                group = spike_group(segid)
                if group and group != selected_spike_group:
                    molecule_segments["Spike"].add(group)
                continue

            if comp == "Mucin" and unit_id:
                molecule_segments["Mucin"].add(unit_id)
            elif comp in {"Albumin", "M Prot", "E Prot"} and unit_id:
    ...
    segment_blocks = {
        ...
        "Albumin": sorted(molecule_segments["Albumin"]),
...
def write_interactome(folder: Path, interactome: str, pdb_files: list[Path], mucin_pairs: dict[str, str]):
    ...
    with counts_txt.open("w") as out_txt, counts_csv.open("w", newline="") as out_csv, segments_file.open("w") as out_segments:
        ...
        for pdb_file in pdb_files:
            ...
            counts, segment_blocks = process_pdb(pdb_file, selected_group, mucin_pairs)
...
def main():
    ...
    parser.add_argument("--base-dir", default=".", help="Directory containing spikeXX folders")
    ...
    if not folders:
        raise SystemExit(f"No spike folders found in {base}")
